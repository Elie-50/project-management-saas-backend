...
describe('Users E2E', () => {
  ...
  describe('PATCH /me', () => {
    it('should update the user info', async () => {
      ...
      // expect(res.body.id).toBe(user.id);
      // expect(res.body.firstName).toBe(updatedUserDto.firstName);
      // expect(res.body.lastName).toBe(updatedUserDto.lastName);
  ...
  describe('DELETE /me', () => {
    it('should delete the user and return 204', async () => {
      ...
      const deleted = await dataSource.getRepository(User).findOneBy({ id: user.id });
