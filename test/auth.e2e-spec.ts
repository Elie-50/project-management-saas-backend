...
describe('Auth E2E', () => {
	...
	describe('Signup', () => {
		...
		it('should fail signup with existing user', async () => {
			await request(httpServer).post(signupUrl).send(userDto);

			await request(httpServer).post(signupUrl).send(userDto).expect(400);
		});
		...
		it('should fail signup with too short password', async () => {
			...
			await request(httpServer).post(signupUrl).send(shortPassUser).expect(400);
		});
	});

	describe('Login', () => {
		beforeEach(async () => {
			await request(httpServer).post(signupUrl).send(userDto).expect(201);
		});
